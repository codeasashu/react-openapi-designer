...
import {NodeTypes, ServiceChildNode, ServiceNode} from './types';
...
export function transformOasToServiceNode(apiDescriptionDocument) {
    ...
    if (match) {
      ...
      if (match.type === NodeTypes.Operation && jsonPath.length === 3) {
        ...
        nodes.push({
          ...
          name:
            operationDocument.summary ||
            operationDocument.iid ||
            operationDocument.path,
...
function findMapMatch(key: string | number, map) {
  if (typeof key === 'number') return;
  for (const entry of map) {
    if (
      !!entry.match?.match(key) ||
      (entry.notMatch !== void 0 && !entry.notMatch.match(key))
    ) {
      return entry;
    }
  }
