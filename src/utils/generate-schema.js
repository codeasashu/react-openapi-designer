// Modules
// Taken from awesome project: https://github.com/nijikokun/generate-schema
...
function getUniqueKeys(a, b, c) {
  ...
  for (
    var keyIndex = 0, keyLength = b.length;
    keyIndex < keyLength;
    keyIndex++
  ) {
    value = b[keyIndex];
...
function processArray(array, output, nested) {
  ...
  for (
    var arrIndex = 0, arrLength = array.length;
    arrIndex < arrLength;
    arrIndex++
  ) {
  ...
  if (typeof output.items.oneOf !== 'undefined' || type === 'object') {
    for (
      var itemIndex = 0, itemLength = array.length;
