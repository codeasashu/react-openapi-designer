...
class ResponseDesigner extends PureComponent {
    ...
    updateState(propertyPath, value) {
        this.setState(
            produce(draft => { set(draft.response, propertyPath, value) })
        );
    }
    ...
    schemaRenderer() {
        ...
        const hasMediaType = !!selectedMediaType && 
            Object.keys(response.content).indexOf(selectedMediaType) >= 0;
        const schema = hasMediaType ? response.content[selectedMediaType]?.schema : null;
        return <SchemaDesigner
                dark
                initschema={schema}
                onChange={
                  e => this.updateState(`content.${selectedMediaType}.schema`, e)
                }
              />
    }
