...
import {
  Intent, ButtonGroup, Button, ControlGroup, InputGroup
} from "@blueprintjs/core";
import { generateHeaderName } from '../utils';
...
class Parameter extends PureComponent {
  ...
  renderRow() {
    ...
    const { disableRequired, ...rest} = this.props;
    return (
      <ControlGroup className="mt-2">
        <InputGroup
          value={name}
          onChange={e => this.setState({ name: e.target.value })}
          onBlur={e => this.handleChange('name')}
          className="flex-auto" placeholder="Header0" 
        />
        ...
        <InputGroup
          ...
          onBlur={e => this.handleChange('description')}
          className="flex-auto" placeholder="Description" 
        />
        <ButtonGroup>
            <Tooltip2
              ...
              <Button
                disabled={disableRequired || false}
                icon="issue"
                onClick={e => this.setState({ required: !required })}
            ...
            <Popover2
              ...
              <AdvancedProperties
                ...
                onChange={e => this.setState(produce(draft => {
                  draft.schema = e
                }))} />
              }
              placement="right">
