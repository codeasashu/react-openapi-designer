...
import {isObject, isEmpty, set, omit, unset} from 'lodash';
...
class Schema {
  ...
  _setupReactions() {
    const {delay} = config.store.schema;
    reaction(
      () => this._lastUpdated,
      action(() => {
...
function* vs(
  ...
  if (M) {
    ...
    if (
      y !== '$ref' &&
      (isTypesInsideTypes(validCombinerTypes, y) ||
        isTypesInsideTypes(validCollectionTypes, y))
    ) {
      l.push(
  ...
  Object.assign(x, {
    ...
    canHaveChildren:
      l.length > 0 ||
      (isTypesInsideTypes(validPrimitiveTypes, h) && !isInside('array', h)) ||
      (isTypesInsideTypes(validPrimitiveTypes, y) &&
        y !== null &&
        !isInside('array', y)),
