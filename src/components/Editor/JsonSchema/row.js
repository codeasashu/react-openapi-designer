...
import {clone, isEqual, pickBy, isEmpty, compact, flattenDeep} from 'lodash';
...
const validNoCombinerTypes = validBasicTypes.concat(validCollectionTypes);
...
const Is = (e, t) => {
  ...
  const r = pickBy(n, (_, n) => t.includes(n));
...
const Os = (
  ...
  if (!(isInside('$ref', t) || isInside('$ref', n))) {
    u = null;
  }
...
const CombinerType = (props) => {
  const {childCount, type, level, rootName, className} = props;
  ...
  if (childCount !== undefined) {
    ...
    return <div className={className}>{e}</div>;
  }

  return null;
};

CombinerType.propTypes = {
...
const SchemaRow = (props) => {
  ...
  const isExtensible = (type, subtype) => {
    if (type) {
      if (
        !(
          isTypesInsideTypes(validParentTypes, type) && !isInside('array', type)
        )
      ) {
        if (isTypesInsideTypes(validParentTypes, subtype)) {
          return !isInside('array', subtype);
        }
      }
      return true;
    }
  ...
  const handleAdd = (node, props) => {
    ...
    if (
      isTypesInsideTypes(validParentTypes, subtype) &&
      !isInside('array', subtype)
    ) {
      u = subtype;
      l = ['children'];
    }
  ...
  const isAdvancedType =
    !isCombinerChild && level !== 0 && isTypesInsideTypes(validTypes, type);
  const isBasicType =
    isTypesInsideTypes(validNoCombinerTypes, type) &&
    type !== '$ref' &&
    type !== 'noType';
  ...
  return (
    <div
      ...
      <div
        ...
        <TooltipButton
          data-testid="move-property-up-button"
        ...
        />
        <TooltipButton
          data-testid="move-property-down-button"
      ...
      {isBasicType && renderOtherProperties(props)}
      <Tooltip
        boundary="window"
        position="top"
        content="Required?"
        ...
        <Button
          data-testid="required-property-button"
      ...
      <Popover
        ...
        target={
          <Tooltip
            ...
            disabled={isRootOrRef}>
            <Button
              data-testid="description-property-button"
...
SchemaRow.propTypes = {
  ...
  node: PropTypes.object,
  type: PropTypes.oneOfType([PropTypes.string, PropTypes.array]),
  subtype: PropTypes.oneOfType([PropTypes.string, PropTypes.array]),
