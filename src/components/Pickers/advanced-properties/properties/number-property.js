...
const NumberProperty = ({data, isSubtype, onChange}) => {
  const changeOtherValue = (value, name) => {
  ...
  const changeEnumOtherValue = (arr) => {
    let cloned = clone(data);
    ...
    <div className="text-sm overflow-auto p-1">
      <div className="pb-6">
        ...
        <div className="flex pb-2">
          <Keywords.Default
            value={data.default || ''}
            onChange={(e) => changeOtherValue(e, 'default')}
          />
          <Keywords.Format
            format={'number'}
            value={data.format}
            onChange={(e) => changeOtherValue(e.target.value, 'format')}
          />
        </div>
        <div className="flex pb-2">
          <Keywords.Enum value={data.enum} onChange={changeEnumOtherValue} />
        </div>
      </div>
      <div>
